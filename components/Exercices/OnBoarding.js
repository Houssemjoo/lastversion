...
function OnBoarding({ userState }) {
    ...
    useEffect(() => {
        ...
        firebase.firestore().collection("exercices").where('usersId', 'array-contains', `${userId}`).get().then((querySnapshot) => {
    ...
    return (
        <View style={styles.container}>
           ...
           <Paginator data={exercicesData} scrollX={scrollX}/>
           {
               ...
               <NextButton scrollTo={scrollTo} percentage={(currentIndex+1)*100/exercicesData.length} /> : null
           }
